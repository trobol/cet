...
#define APP_USE_VULKAN_DEBUG_REPORT 1
...
#define VK_CHECK(ret) check_vk_result(ret);
...
static void SetupVulkan(ImVector<const char*> instance_extensions)
{
    ...
    // Create Logical Device (with 1 queue)
    {
        ...
        VK_CHECK(vkCreateDevice(g_PhysicalDevice, &create_info, g_Allocator, &g_Device));
    ...
    // If you wish to load e.g. additional textures you may need to alter pools sizes.
    {
        ...
        pool_info.pPoolSizes = pool_sizes;
       	VK_CHECK( vkCreateDescriptorPool(g_Device, &pool_info, g_Allocator, &g_DescriptorPool) );
    }
}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR pCmdLine, int nCmdShow)
{
	...
	VK_CHECK( vkCreateWin32SurfaceKHR( g_Instance, &createInfo, g_Allocator, &surface ) );

	ImGui_ImplVulkanH_Window* wd = &g_MainWindowData;
	SetupVulkanWindow(wd, surface, 50, 50);
	...
	while (!done)
	{
		...
		ImGui_ImplVulkanH_CreateOrResizeWindow(g_Instance, g_PhysicalDevice, g_Device, &g_MainWindowData, g_QueueFamily, g_Allocator, area.right, area.bottom, g_MinImageCount);
