// copy of Child.zig from std
// many of the functions I want to use are private
...
pub fn spawn(self: *ChildProcess, out: fs.File, in: fs.File ) SpawnError!void {
    if (!process.can_spawn) {
        @compileError("the target operating system cannot spawn processes");
    }

    if (native_os == .windows) {
        return self.spawnWindows( out.handle, in.handle );
    } else {
...
pub fn nullHandle() windows.OpenError!fs.File
{
	...
	// "\Device\Null" or "\??\NUL"
	const handle = windows.OpenFile(&[_]u16{ '\\', 'D', 'e', 'v', 'i', 'c', 'e', '\\', 'N', 'u', 'l', 'l' }, .{
...
fn spawnWindows(self: *ChildProcess, pipe_out: windows.HANDLE, pipe_in: windows.HANDLE ) SpawnError!void {
...
pub fn windowsDestroyPipe(rd: ?windows.HANDLE, wr: ?windows.HANDLE) void {
    if (rd) |h| posix.close(h);
    if (wr) |h| posix.close(h);
}

pub fn windowsMakeAsyncPipe(rd: *?windows.HANDLE, wr: *?windows.HANDLE, sattr: *const windows.SECURITY_ATTRIBUTES) !void {
